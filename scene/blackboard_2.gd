...
var worker3: CharacterBody2D
var worker4: CharacterBody2D
var object=["stick","stick","rock","stick"]
...
func _ready():
	...
	worker3 = get_parent().get_node("Player2")
	worker4 = get_parent().get_node("Player3")
	shuffle_object()
...
func request_resource_worker_1():
	...
	if worker and !worker2.is_collecting() and !worker3.is_collecting() and !worker4.is_collecting()   and worker.has_method("actor_setup") and object.size()==object_en_possession and bb.object.size()==bb.object_en_possession:
...
func request_resource_worker_2():
	...
	if worker2 and !worker.is_collecting() and !worker3.is_collecting() and !worker4.is_collecting()  and worker2.has_method("actor_setup") and object.size()==object_en_possession and bb.object.size()==bb.object_en_possession:
