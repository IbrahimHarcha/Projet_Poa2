...
var object=["rock","rock","stick","rock"]
...
func _ready():
	bb=get_parent().get_node("blackboard_2")
	worker = get_parent().get_node("Player2")
	worker2 = get_parent().get_node("Player3")
	worker3 = get_parent().get_node("Player4")
	worker4 = get_parent().get_node("Player5")
	shuffle_object()
...
func request_resource_worker_1():
	...
	if worker and !worker2.is_collecting()   and worker.has_method("actor_setup") and object.size()==object_en_possession and bb.object.size()==bb.object_en_possession:
...
func request_resource_worker_2():
	...
	if worker2 and !worker.is_collecting()  and worker2.has_method("actor_setup") and object.size()==object_en_possession and bb.object.size()==bb.object_en_possession:
